...
One more thing that needs to do to use our model instead of their model is to change the model. Their model has been trained on GCP, but our model is not. So in step 3 [Serving predictions from a custom image classification model](https://cloud.google.com/vertex-ai/docs/tutorials/image-recognition-custom/serving), we will not use the model that has been trained, we will import our model by clicking import button and follow the instruction. Finally, follow the remaining steps to create end-point and we will also end up with a link like the previous one. 
